...
def genSphere(z,pxlen,centre,R, semi=False):
        ...
        for x in range(lwrx,uprx):
            for y in range(lwry,upry):
                if R[i]**2 - (x*pxlen - x0)**2 - (y*pxlen - y0)**2 > 0:
                    z[y,x] += np.sqrt(R[i]**2 - (x*pxlen - x0)**2 - (y*pxlen - y0)**2)
                    if not(semi): z[y,x] += R[i]
    return z
...
def genHexCap(z, pxlen, hmin, hmax, dist, rmin, rmax, elem='cap', **kwargs):
    ...
    while xmin<xmax:
        ...
        while y0<ymax:
            ...
            if lwrx<0: lwrx=0         #tutta la mappa, importante se ho
            uprx=int((xmin+R)/pxlen)+2  #alta risoluzione
            if uprx>len(z): uprx=len(z)
            ...
            for x in range(lwrx,uprx):
                for y in range(lwry,upry):
                    if R**2 - (x*pxlen - xmin)**2 - (y*pxlen - y0)**2 > 0:
                        z[y,x]+=np.sqrt(R**2 - (x*pxlen - xmin)**2 - (y*pxlen - y0)**2)
                        if elem=='sphere' or elem=='particle': z[y,x]+=R
            y0+=dist
...
def plotfalsecol(z,pxlen):
    plt.figure()
    plt.title(r'$L_{px}=$'+str(round(pxlen,3)))
    plt.axis('equal')
